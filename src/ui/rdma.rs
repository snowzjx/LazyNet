use crate::collectors::rdma::RdmaPort;
...
fn draw_list(
    ...
    let rows: Vec<Row> = devices
        ...
        .map(|node| {
            let name = node
                .get_property("display_name")
                .or_else(|| node.get_property("name"))
                .cloned()
                .unwrap_or_else(|| "N/A".into());
            let transport = node
                .get_property("transport")
                .cloned()
...
fn draw_detail(
    ...
    let port = RdmaPort::from_node(node);
    let name = port
        .as_ref()
        .map(|p| p.label())
        .unwrap_or_else(|| node.get_property("name").cloned().unwrap_or_default());
    ...
    let mut pfc_lines = vec![
        ...
        Line::from(vec![
            Span::styled("NetDev:  ", Style::default().add_modifier(Modifier::BOLD)),
            Span::raw(netdev.clone().unwrap_or_else(|| "-".into())),
        ]),
        Line::from(""),
    ];
    if let Some(pfc) = pfc {
        pfc_lines.push(Line::from(vec![
            Span::styled("PFC Cap: ", Style::default().add_modifier(Modifier::BOLD)),
            Span::raw(pfc.pfc_cap.to_string()),
        ]));
        pfc_lines.push(Line::from(""));
        pfc_lines.push(Line::from(Span::styled(
            "Prio  PFC",
        ...
        pfc_lines.push(Line::from(format!(
            "  RX: {}  TX: {}",
            pfc.rx_pause, pfc.tx_pause
        )));
    } else {
        pfc_lines.push(Line::from(Span::styled(
            "PFC data unavailable",
    ...
    let (rdma_c, counter_title) = match recording {
        Some(CounterRecording::Started(_)) => {
            (None, format!("Port Counters [RECORDING] — {}", name))
        }
        Some(CounterRecording::Finished { .. }) => (
            port.as_ref()
                .and_then(|p| recording.unwrap().rdma_delta(&p.key())),
            format!(
                "Port Counters [DELTA {:.1}s] — {}",
                recording.unwrap().delta_secs(),
                name
            ),
        ),
        None => (
            port.as_ref()
                .and_then(|p| inventory.rdma_counters.get(&p.key()).cloned()),
...
fn stat_line(label: &'static str, val: u64, is_error: bool, is_delta: bool) -> Line<'static> {
    let style = if is_error && is_delta && val > 0 {
        Style::default().fg(Color::Red)
    } else {
        Style::default()
    };
...
fn build_port_lines(
    ...
    let is_delta = recording
        .map(|r| matches!(r, CounterRecording::Finished { .. }))
        .unwrap_or(false);
...
fn build_roce_lines(
    ...
    vec![
        ...
        stat_line(
            "Transport retries: ",
            c.req_transport_retries_exceeded,
            true,
            is_delta,
        ),
        stat_line(
            "Local ACK timeout: ",
            c.local_ack_timeout_err,
            true,
            is_delta,
        ),
        stat_line(
            "ICRC errors:       ",
            c.rx_icrc_encapsulated,
            true,
            is_delta,
        ),
    ]
